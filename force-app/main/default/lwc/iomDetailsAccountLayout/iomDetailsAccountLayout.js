...
import SYNC_STATUS from '@salesforce/schema/Account.IOM_Sync_Status__c';
import OPP_NO from '@salesforce/schema/Account.IOM_Account_No__c';


export default class IomDetailsAccountLayout extends LightningElement {
    ...
    wiredRecord({ error, data }) {
        if (error) {
            ...
            // console.log('VAl2==>'+data.fields.IOM_Account_No__c.value);
            if(data.fields.IOM_Sync_Status__c.value=='Complete'){
                this.showIOMOpporunity = true;
            }
            if(data.fields.IOM_Sync_Status__c.value=='Complete' && data.fields.IOM_Account_No__c.value!=''){
