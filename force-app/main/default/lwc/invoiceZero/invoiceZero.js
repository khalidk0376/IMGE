...
export default class InvoiceNonZero extends LightningElement {
  ...
  @api fields =
    "Name,Invoice_Number_SFDC__c,blng__InvoiceStatus__c,blng__TotalAmount__c,Event_Edition_Name__c,blng__InvoiceDate__c,blng__Account__r.Name,CreatedDate";
  @api fieldsLabel =
    "Invoice Number,Sequential Number,Status,Total Amount (With Tax),Event Edition Name, Invoice Date,Account, Created Date";
  @api condition = "Name!=''";
  @track tempCondition = "";
  @api profile = "";
    ...
    getSSCTeamMember()
      .then(result => {
        if (result) {
          ...
          this.condition = '(blng__InvoiceStatus__c=\'Draft\' AND blng__Order__r.Opportunity.Event_Series__c IN (\'' + ids.join('\',\'') + '\') AND blng__TotalAmount__c=0.0)';
          this.showTable = true;
