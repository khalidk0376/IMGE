...
import { exportCSV } from 'c/lWCUtility';

export default class EventsAllBoothsTab extends LightningElement {
    @api mapBoothAllEvents;
    @track mapBoothAllEventsTempVar;
    @api allBoothColumns;
    ...
    setData(){
        if(this.mapBoothAllEvents){
            this.pasesizeVisible = this.pagesize.toString();
            this.totalRows = this.mapBoothAllEvents.length;
            ...
            for(let i=startIndex;i<endIndex;i++){
                this.mapBoothAllEventsTempVar.push(this.mapBoothAllEvents[i]);
            }

            if(this.totalRows===0){                
                this.mapBoothAllEventsTempVar=undefined;
    ...
    */
    downloadCSVFile(){
        let requiredlabels = ['Area','BoothClasses','BoothNumber','BoothType','Dimensions','DisplayNameOverride','AccountName','IsOnHold','IsRented','OpenCorners','OpenSides','Pavilion','ShowInShow'];
        if(this.selectedRows){
            exportCSV(this.selectedRows,requiredlabels,'HeldBooth-export');
        } else {
            exportCSV(this.mapBoothAllEvents,requiredlabels,'HeldBooth-export');
        }
    }
