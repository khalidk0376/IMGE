/*
Created By	 : Girikon(Himanshu[STL-45])
...
export default class CommonTable extends LightningElement {
    ...
    @api sortByFieldName = 'LastModifiedDate';
    ...
    handleRowAction(event) {
        ...
        switch (actionName) {
            ...
            case 'openlink':
                ...
                break;
            case 'fileopen':
                    this.dispatchEvent(new CustomEvent('fileopen', { detail: row }));
                break;
            case 'openlinkbutton':
    ...
    setParentFieldColumn(columnObj,columnList,datas){
        ...
        if(columnObj.length > 0 && columnObj[0].label==='Stand Design Attachments Name'){
            columnObj[0].label = 'Stand Design Attachments';
        }
        ...
        if (this.openLinkIcon2) {            
            columnObj.splice(0, 0,
                {
                    type: "button-icon",
                    initialWidth: 25,
                    typeAttributes: {
                        iconName: this.openLinkIcon2 ? this.openLinkIcon2 : 'utility:forward_up',
                        name: 'fileopen',
                        title: this.openLinkText2,
                        variant: 'bare',
                        disabled: false,
                        value: { fieldName: 'Id' }
                    }
        ...
        for (let i = 0; i < columnObj.length; i++) {
            ...
            if (columnObj[i].type === 'currency') {
                if (columnList.toLowerCase().indexOf('currencyisocode') > 0) {
                    columnObj[i].typeAttributes = { currencyDisplayAs: 'code', currencyCode: { fieldName: 'CurrencyIsoCode' } };
                }
    ...
    openLink(row) {
        this.dispatchEvent(new CustomEvent('openlink', { detail: row.Id }));
        this.dispatchEvent(new CustomEvent('openlinkmodal', { detail: row }));
    }
    ...
    closeDateFilter() {
        ...
        //this.dateFilterApplied = !this.dateFilterApplied;
    }
