...
export default class BaReviewRequiredDashboard extends LightningElement {
    ...
    //filter1
    @api filterField1 = "blng__Order__r.Opportunity.EventEdition__r.Name";
    @api filter1Label = "Event Edition";
    ...
    @track isMassUpdateInvoice = false;
      ...
      getSSCTeamMember()
        .then(result => {
          if (result) {
            ...
            this.condition ='blng__Order__r.Opportunity.Event_Series__c IN (\''+ids.join('\',\'')+'\') AND BA_review_Required__c = true AND BA_approved__c= false AND blng__Order__r.OpportunityId!=null';
            this.showTable = true;
      ...
      massUpdateRecords({ objList: invoiceIds })
        .then(result => {
          if (result) {
            ...
            this.getData();
            this.isMassUpdateInvoice = false;
          }
        })
        .catch(error => {
          this.spinner = false;
