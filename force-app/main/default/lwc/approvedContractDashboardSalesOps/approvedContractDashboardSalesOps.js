    ...
    getSSCTeamMember()
      .then(result => {
        if (result) {
          ...
          this.pendingContractCond ='(Status__c=\'Pending Accounting Approval\' AND Exhibitor_Paid_By__c != \'Delegate Sales\' AND EventEdition__r.Part_of_Series__c IN (\''+ids.join('\',\'')+'\') AND Do_not_activate_Billing__c = false) AND (No_Billing__c = false OR (EventEdition__r.No_Billing__c = true AND EventEdition__r.Event_Price_Book__r.Name = \'Brazil\'))';

          this.toggleDelegateOppCon = '(Status__c=\'Pending Accounting Approval\' AND Exhibitor_Paid_By__c = \'Delegate Sales\' AND EventEdition__r.Part_of_Series__c IN (\''+ids.join('\',\'')+'\') AND Do_not_activate_Billing__c = false) AND (No_Billing__c = false OR (EventEdition__r.No_Billing__c = true AND EventEdition__r.Event_Price_Book__r.Name = \'Brazil\'))';
    ...
    .then(res=>{
        if(res.length > 0){
            if(res[0].UserRole.Name === 'SSC-Brazil'){
                this.oppFields = 'Name,Account.Name,EventEdition__r.Name,CloseDate,Submission_Date__c,StageName,Status__c,Tax_Rule_SAP__c,Amount,Approved_Rejected_By_Sales_Ops__r.Name,Approved_Rejected_At_Sales_Ops__c,Owner.Name';
                this.oppFieldsLabels = 'Opportunity Name,'+Account_Name+',Event Edition,Close Date,'+Submission_Date+',Stage,Status,TAX Rule,Amount,Approved By,Approved At,'+Opportunity_Owner;
            } else if(res[0].UserRole.Name !== 'SSC-Brazil') {
                this.oppFields = 'Name,Account.Name,EventEdition__r.Name,CloseDate,StageName,Status__c,Tax_Rule_SAP__c,Amount,Approved_Rejected_By_Sales_Ops__r.Name,Approved_Rejected_At_Sales_Ops__c,Owner.Name';
                this.oppFieldsLabels = 'Opportunity Name,'+Account_Name+',Event Edition,Close Date,Stage,Status,TAX Rule,Amount,Approved By,Approved At,'+Opportunity_Owner;
            }
        }
    })
    .catch(error=>{
