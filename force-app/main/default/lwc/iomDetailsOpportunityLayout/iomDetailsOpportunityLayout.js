...
import SYNC_STATUS from '@salesforce/schema/Opportunity.IOM_Sync_Status__c';
import OPP_NO from '@salesforce/schema/Opportunity.IOM_Opportunity_No__c';


export default class IomDetailsOpportunityLayout extends LightningElement {
    ...
    wiredRecord({ error, data }) {
        if (error) {
            ...
            // console.log('VAl2==>'+data.fields.IOM_Opportunity_No__c.value);
            if(data.fields.IOM_Sync_Status__c.value=='Complete'){
                this.showIOMOpporunity = true;
            }
            if(data.fields.IOM_Sync_Status__c.value=='Complete' && data.fields.IOM_Opportunity_No__c.value!=''){
