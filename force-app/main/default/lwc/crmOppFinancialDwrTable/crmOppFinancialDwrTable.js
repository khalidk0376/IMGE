...
export default class CrmOppFinancialDwrTable extends LightningElement {
    //action properties
    ...
    @api objectLabel='Order';    
    @api fields='Id,OrderNumber,Name,Account.Name,Opportunity.Name,SBQQ__Quote__c,SBQQ__Quote__r.Name,Status,ActivatedDate,Contract.ContractNumber,Main_Order__c';
    @api fieldsLabel='Id,Order Number,Name,Account Id,Opportunity Id,Quote,Status,Activated Date,Contract Number,Main Order';
    @api condition;
    @track tempCondition='';
    @api profile='';    
    ...
    retrieveCurrentUserProfile(){
        ...
        .then(result=>{
            if(result){
                if(result.length!==0){
                    if(result[0].Profile.Name === 'System Administrator' || result[0].Profile.Name === 'GE System Administrator' || result[0].Profile.Name === 'GE BA Administrator' || result[0].Profile.Name === 'SSC Finance-Accounting'  ){
                        this.isAdminProfile = true;
                        // eslint-disable-next-line no-alert
                        //alert(this.isAdminProfile);
                    }
                }
            }
        })
    ...
    setParentFieldValue(tbldatas){ 
        ...
        for(let i=0;i<datas.length;i++){
            ...
            datas[i].orderLink = '/lightning/r/'+this.objectName+'/'+datas[i].Id+'/view'
            //build link
