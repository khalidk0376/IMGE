/*
    ...
    @description 	: This ControllerJS is of declined Opportunity.
...
//custom label retrived
import OPP_NAME from '@salesforce/label/c.oppName';
import COMMENT from '@salesforce/label/c.comment';
import MODIFIED_DATE from '@salesforce/label/c.Last_Modified_Date';
...
export default class DeclinedOpportunityDashboard extends LightningElement {
    //custom label
    @track oppName = OPP_NAME;
    @track commentCol = COMMENT;
    @track Last_Modified_Date = MODIFIED_DATE;
    ...
    @track tableData;
    @track tableColumn;
    @api objectName='Booth_Approval_Dashboard__c';
    @api objectLabel='Booth Approval Dashboard';    
    @api fields='Opportunity__r.Name,Opportunity__r.Event_Series__r.Name,Opportunity__r.EventEdition__r.Name,Status__c,Comment__c,LastModifiedDate';
    @api fieldsLabel='Id,Name';
    @api condition='Opportunity__r.Booth_Approval_Status__c=\'Declined\' AND Opportunity__r.MSA_Requested__c=false AND Status__c=\'Declined\'';// AND Opportunity__r.EventEdition__r.Event_Director__c = \''+userId+'\'';
    @track tempCondition='Opportunity__r.Booth_Approval_Status__c=\'Declined\' AND Opportunity__r.MSA_Requested__c=false AND Status__c=\'Declined\'';// AND Opportunity__r.EventEdition__r.Event_Director__c = \''+userId+'\'';
    @api profile='';    
    ...
    */
    setParentFieldValue(tbldatas){ 
        ...
        for(let i=0;i<datas.length;i++){
            ...
            datas[i].condition = 'Opportunity__r.Booth_Approval_Status__c=\'Declined\' AND Opportunity__c = \''+datas[i].Opportunity__r.Id+'\'';
            //build link
