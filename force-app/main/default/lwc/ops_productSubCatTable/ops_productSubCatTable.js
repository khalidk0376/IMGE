...
export default class Ops_productSubCatTable extends LightningElement {
    //action properties
    ...
    connectedCallback(){
        ...
        this.pagesize = 50;
        this.offst = 0;
        this.objectName ='Event_Edition_Sub_Category_Mapping__c';
    ...
    getData(){
       ...
       this.tempCondition = 'Event_Edition_Product_Category_Mapping__c=\''+ this.productId +'\'' ;
    ...
    yesDeleteSingleRecord() {
            var records = [];
            let recID = this.deleteRecordId;
            let recordid = recID.substring(1);
            records.push({ Id:recordid.toString(), sobjectType: this.objectName });
            this.spinner = true;
            deleteRecord({objList:records})
            .then(res => {
                this.spinner = false;
    ...
    DeleteModel(event){
        ...
        let recID = rectarget.getAttribute('data-recid');
