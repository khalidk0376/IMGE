...
/*
Created By	 : Girikon(Mukesh[125])
Created On	 : 4 Sept, ‎2019
@description : Display "Interest level" and provide add/edit/delete features on opportunity lead info tab.
...
export default class CrmInterestLevel extends LightningElement {
    //Custome label    
    ...
    connectedCallback(){
        ...
        .then(res=>{
            this.condition = 'SFDCOpportunityID__c=\''+this.recordId+'\' ';  
    ...
    submitData(){        
        ...
        this.interestLevel.forEach(item => {
            if(item.fieldValue1!==''){
                objList.push({sobjectType:'LeadInterestLevelMapping__c',SFDCOpportunityID__c:this.recordId,SFDCAccountID__c:this.oppAccountid,SFDCContactID__c:this.oppContactid, L1__c:item.fieldValue1,L2__c:item.fieldValue2,L3__c:item.fieldValue3});
            }
        });
        if(objList.length>0){
            this.spinner = true;
    ...
    updateBulkData(){        
        let objList = [];
        this.interestLevel.forEach(item => {            
            objList.push({sobjectType:'LeadInterestLevelMapping__c',Id:item.Id,SFDCOpportunityID__c:this.recordId,L1__c:item.fieldValue1,L2__c:item.fieldValue2,L3__c:item.fieldValue3});
        });
        if(objList.length>0){
            this.spinner = true;
            window.clearTimeout(this.delayTimeout);
