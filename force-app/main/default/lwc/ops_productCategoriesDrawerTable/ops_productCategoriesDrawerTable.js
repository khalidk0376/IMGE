...
export default class DrawerTable extends LightningElement {
    ...
    connectedCallback(){
        ...
        this.pagesize = 50;
        this.offst = 0;
        this.objectName ='Event_Edition_Product_Category_Mapping__c';
        this.fields ='Levels_Master__r.LevelValue__c';
        const col = [];
    ...
    getData(){
        ...
        this.tempCondition = 'Event_Edition__c=\''+ this.eventId +'\' AND Levels_Master__r.Level_Name__c = \'L2\'';
    ...
    yesDeleteSingleRecord(event) {
       
        
        let rectarget = event.currentTarget;
        let recID = rectarget.getAttribute('data-id');
        this.recordIdForDelete = recID.substring(1);
  ...
  DeleteproductRecord(){
    var records = [];
    records.push({ Id:this.recordIdForDelete, sobjectType: this.objectName });
        this.spinner = true;
        deleteRecord({objList:records})
        .then(res => {
            this.spinner = false;
   ...
   setFilterOptions(filterNum,fieldName){
    ...
    toggleDetail(event){
        try
        {
            this.tableData[event.target.dataset.rowno].isShowDetail=true;
