    ...
    */
    getSSCTeamMember(){
        ...
        .then(data=>{
            ...
            this.pendingContractCond = '(Status__c = \'Pending Accounting Approval\' AND Exhibitor_Paid_By__c != \'Delegate Sales\' AND EventEdition__r.Part_of_Series__c IN (\''+ids.join('\',\'')+'\') AND SBQQ__Contracted__c = true AND Main_Contract__c != NULL AND Do_not_activate_Billing__c = false AND IsAmendContractOpp__c = false) AND (No_Billing__c = false OR (EventEdition__r.No_Billing__c = true AND EventEdition__r.Event_Price_Book__r.Name = \'Brazil\') )';

            this.toggleDelegateOppCon = '(Status__c = \'Pending Accounting Approval\' AND Exhibitor_Paid_By__c = \'Delegate Sales\' AND EventEdition__r.Part_of_Series__c IN (\''+ids.join('\',\'')+'\') AND SBQQ__Contracted__c = true AND Main_Contract__c != NULL AND Do_not_activate_Billing__c = false AND IsAmendContractOpp__c = false) AND (No_Billing__c = false OR (EventEdition__r.No_Billing__c = true AND EventEdition__r.Event_Price_Book__r.Name = \'Brazil\') )';
        ...
        .then(res=>{
            if(res.length > 0){
                if( res[0].UserRole.Name === 'SSC-Brazil'){
                    this.oppFields = 'Name,Account.Name,ISO_Code__c,Amount_Custom__c,SBQQ__PrimaryQuote__r.Nota_Fiscal_Due__c,CloseDate,Submission_Date__c,StageName,Status__c,Tax_Rule_SAP__c,SSC_Brazil_Status__c,Rejection_Responses__c,SSC_Notes__c,EventEdition__r.Name';
                    this.oppFieldsLabels = 'Opportunity Name,'+Account_Name+',ISO Code,Contract Total,Amount to Pay,Close Date,'+Submission_Date+',Stage,Status,TAX Rule,SSC Brazil Status,Rejection Responses,SSC Notes,Event Edition';
                } else if( res[0].UserRole.Name !== 'SSC-Brazil') {
                    this.oppFields = 'Name,Account.Name,ISO_Code__c,Amount_Custom__c,CloseDate,StageName,Status__c,Pending_Accounting_Approval_Since_Date__c,Tax_Rule_SAP__c,SSC_Brazil_Status__c,Rejection_Responses__c,SSC_Notes__c,EventEdition__r.Name';
                    this.oppFieldsLabels = 'Opportunity Name,'+Account_Name+',ISO Code,Contract Total,Close Date,Stage,Status,Pending Accounting Approval Since Date,TAX Rule,SSC Brazil Status,Rejection Responses,SSC Notes,Event Edition';
                }
            }
        })
        .catch(error=>{
