/*
Created By	 : (Ramkrishna)
Created On	 : Sep 5, 2019
@description : This component is use to show Viewded Report under Form Settings in Customer Center.
...
import DocType from '@salesforce/schema/Event_Edition_Form__c.Form_Type__c';
import BusinessUnit from '@salesforce/schema/Form__c.Business_Unit__c';
import FormGroup from '@salesforce/schema/Form__c.Form_Group__c';
import FormDescription from '@salesforce/schema/Form__c.Form_Description__c';
import Url from '@salesforce/schema/Form__c.Url__c';
...
const FIELDS = ['Event_Edition__c.Event_Code__c'];

const DELAY = 300;
export default class Ops_reportViewed extends NavigationMixin(LightningElement) {
    label = { UploadMessage };
    formObject = Form;
    formName = Name;
    docType = DocType;
    ...
    @api objectLabel = 'Forms Permission';
    @api fields = 'Account__r.Name,User_Type__r.Name';
    ...
    @track downloadableLinkTypeTemplate=false;
    @track iframeLinkValue=''
    @track inputDisabled=true
    @track eventEditionId=''
    @track eventCode=''
    @api whereCallFrom = '';
    @api objSelectedViewedFromParent={showStatus:false};

    connectedCallback() {
        console.log('---------cal connected')
        ...
        if (fullUrl.indexOf("#") > -1) {
          let  tempEventEditionId=fullUrl.split("#id=")[1];   
            if(tempEventEditionId.indexOf("&esid=")){
                tempEventEditionId=tempEventEditionId.split("&esid=")[0];
            }
            this.eventEditionId=tempEventEditionId;
            console.log('---------------this.eventEditionId'+this.eventEditionId);
            //this.condition="Event_Edition_Form__r.Event_Edition__c='"+this.eventEditionId+"' AND Active__c = true"
        }

    }
    @wire(getRecord, { recordId: '$eventEditionId',fields: FIELDS })
    ...
    getData() {
        ...
        getDatas({
            ...
            next: this.hasNext, prev: this.hasPrev, off: this.offst, sortBy: this.sortByName, sortType: this.sortType, condition: this.condition
        })
            .then(data => {
                if (this.offst === -1) {
                    this.offst = 0;
    ...
    handleUploadFinished(event) {
        ...
        updateAttachOnForms({ formId: record, attId: attachId, attName: attachmentName })
            .then(result => {
                window.console.log(' updateAttachOnForms----' + result);
            })
            .catch(error => {
                window.console.log("error..." + JSON.stringify(error));
            });
    ...
    handleEditUploadFinished(event) {
        ...
        updateAndDeleteAttOnForm({
            ...
            .then(result => {
                window.console.log('edit attach' + result);
            })
            .catch(error => {
                window.console.log("error..." + JSON.stringify(error));
            });
    ...
    handleformSubmit(event) {
        ...
        let doctype = fields.Form_Type__c;
...
viewForm(event){
    ...
    let formType=this.selectedRecordView.Form_Type__c;
        if(formType==='Online'){
            this.iframeLinkValue='/apex/c__FormPreviewOpsAdmin?Id=' +this.selectedRecordView.Template_Form__c+'&eventCode='+this.eventCode;
        this.onLineTypeTemplate=true;  
        this.linkTypeTemplate=false;
        this.downloadableLinkTypeTemplate=false;   

        }else if(formType==='Link'){ 
            this.iframeLinkValue=this.selectedRecordView.Forms__r.Url__c;  

            this.onLineTypeTemplate=false;  
            this.linkTypeTemplate=true;
            this.downloadableLinkTypeTemplate=false;  
        } else if(formType==='Downloadable PDF'){
            this.iframeLinkValue='/servlet/servlet.FileDownload?file=' +this.selectedRecordView.Forms__r.Uploaded_Attachment_Id__c;            
        
            this.onLineTypeTemplate=false;  
        this.linkTypeTemplate=false;
        this.downloadableLinkTypeTemplate=true;  
        }
        // eslint-disable-next-line @lwc/lwc/no-async-operation
        this.delayTimeout = setTimeout(()=>{
...
openOnlineFile(event){
    ...
    this[NavigationMixin.Navigate]({
        ...
        attributes: {
            url: '/apex/RedirectUrlForEvent?tempId='+this.selectedRecordView.Template_Form__c
        },
    });
}
closeRightSection(){
    let tempObj={...this.objSelectedViewedFromParent}
    tempObj.showStatus=false;
    this.objSelectedViewedFromParent=tempObj;
}
}