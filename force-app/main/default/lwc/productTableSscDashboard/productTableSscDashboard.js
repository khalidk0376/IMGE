...
const DELAY=300;
export default class ProductTableSscDashboard extends LightningElement {
    //action properties
   ...
   @api isMultiLanguage='false';
    ...
    getData(){
        try{
            if(this.oppId && this.objectName==='SBQQ__QuoteLine__c'){
                this.tempCondition = this.tempCondition +' AND SBQQ__Quote__r.SBQQ__Opportunity2__c=\''+this.oppId+'\'';
            }
            this.spinner = true;
            getDatas({searchValue:this.searchValue,objectName:this.objectName,fieldstoget:this.fields,pagesize:this.pagesize,
                next:this.hasNext,prev:this.hasPrev,off:this.offst,sortBy:this.sortByName,sortType:this.sortType,condition:this.tempCondition,isAggregate:true})
            .then(data=>{            
                if(this.offst === -1){
                    this.offst = 0;
                }
                ...
                this.totalAmount = numberFormat.format(data.aggregateAmount);
    ...
    setParentFieldColumn(columnObj,columnList,datas){
        ...
        for(let i=0;i<columnObj.length;i++){
            ...
            if(columnObj[i].fieldName!==undefined && 
                ...
                && columnObj[i].fieldName.toLowerCase()!=='work_order_name__c' && columnObj[i].fieldName.toLowerCase()!=='task_name__c' && columnObj[i].fieldName.toLowerCase()!=='sbqq__productname__c' && columnObj[i].fieldName.toLowerCase()!=='display_name__c' && columnObj[i].fieldName.toLowerCase()!=='account_name__c' && columnObj[i].fieldName.toLowerCase()!=='asigneename__c'){
