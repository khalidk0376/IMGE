...
@Description:   Generic trigger on SSC_Team_Member__c which should call from Trigger app for any alteration of any SSC Team Member record.
Coverage    :   94%
Test Class  :   OperationsTeamMemberValidation_Test
*/
trigger Trigger_SSCTeamMember on SSC_Team_Member__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) 
{
    TriggerInterface_SscTeamMember.Trig trig = new TriggerInterface_SscTeamMember.Trig();
    ...
    TriggerInterfaceHelper.initTriggerMapAndList('SSC_Team_Member__c');
    ...
    { 
        ...
        if (t.Object__c == 'SSC_Team_Member__c') 
        {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             {
                trig.oldList = new List<SSC_Team_Member__c>();
                trig.newList = new List<SSC_Team_Member__c>();
                trig.oldMap = new Map<Id,SSC_Team_Member__c>();
                trig.newMap = new Map<Id,SSC_Team_Member__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) 
                {
                    ...
                    {
                        TriggerInterface_SscTeamMember.Runnable runnableClass;
                        try 
                        {
                            runnableClass = (TriggerInterface_SscTeamMember.Runnable) runnableType.newInstance();
                        } 
                        catch(Exception e) 
                        {
                        ...
                        if (runnableClass != null) 
                        {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) 
                            {
                                ...
                                system.debug('********** Trigger_SSCTeamMember (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_SSCTeamMember (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)
