...
Description/Purpose: This trigger is use for updating total badges if any changes made on booth size badge limit record.
*/
trigger Trigger_BoothSizeBadgeLimit on Booth_Size_Badge_Limit__c(before insert, before update, before delete, after insert, after update, after delete, after undelete) {
    
    TriggerInterface_BoothSizeBadgeLimit.Trig trig = new TriggerInterface_BoothSizeBadgeLimit.Trig();
    ...
    TriggerInterfaceHelper.initTriggerMapAndList('Booth_Size_Badge_Limit__c');
    ...
    for (Trigger__c t : listTriggers) {   
        ...
        if (t.Object__c == 'Booth_Size_Badge_Limit__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<Booth_Size_Badge_Limit__c>();
                trig.newList = new List<Booth_Size_Badge_Limit__c>();
                trig.oldMap = new Map<Id,Booth_Size_Badge_Limit__c>();
                trig.newMap = new Map<Id,Booth_Size_Badge_Limit__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_BoothSizeBadgeLimit.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_BoothSizeBadgeLimit.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** TriggerInterface_BoothSizeBadgeLimit(' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)
