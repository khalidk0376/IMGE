...
@Description: Generic trigger on Event_Edition__c which should call from Trigger app for any alteration of any Event Edition's record.
Test Class:   BadgesAssignmentByBoothType_Test 
*/
trigger Trigger_EventEdition on Event_Edition__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) 
{
    TriggerInterface_EventEdition.Trig trig = new TriggerInterface_EventEdition.Trig();
    ...
    TriggerInterfaceHelper.initTriggerMapAndList('Event_Edition__c');
    ...
    { 
        ...
        if (t.Object__c == 'Event_Edition__c') 
        {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             {
                trig.oldList = new List<Event_Edition__c>();
                trig.newList = new List<Event_Edition__c>();
                trig.oldMap = new Map<Id,Event_Edition__c>();
                trig.newMap = new Map<Id,Event_Edition__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) 
                {
                    ...
                    {
                        TriggerInterface_EventEdition.Runnable runnableClass;
                        try 
                        {
                            runnableClass = (TriggerInterface_EventEdition.Runnable) runnableType.newInstance();
                        } 
                        catch(Exception e) 
                        {
                        ...
                        if (runnableClass != null) 
                        {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) 
                            {
                                ...
                                system.debug('********** Trigger_evntEdition (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_evntEdition (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)
