...
*
* *************************************************************************
* @description "Booth_Approval_Dashboard__c " Trigger.
* *************************************************************************
* History:Ticket :GGCkB-76
*
*/

trigger Trigger_BoothApprovalDashboard on Booth_Approval_Dashboard__c (Before Insert, Before Update, Before Delete, After Insert, After Update, After Delete, After Undelete) {
    
    TriggerInterface_BoothApprovalDashboard.Trig trig = new TriggerInterface_BoothApprovalDashboard.Trig();
    ...
    TriggerInterfaceHelper.initTriggerMapAndList('Booth_Approval_Dashboard__c');
    ...
    for (Trigger__c t : listTriggers){
        ...
        if (t.Object__c == 'Booth_Approval_Dashboard__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
                ...
                || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                    trig.oldList = new List<Booth_Approval_Dashboard__c>();
                    trig.newList = new List<Booth_Approval_Dashboard__c>();
                    trig.oldMap = new Map<Id,Booth_Approval_Dashboard__c>();
                    trig.newMap = new Map<Id,Booth_Approval_Dashboard__c>();
                    trig.triggerSize = 0;
                    ...
                    if (trig.triggerSize > 0) {
                        ...
                        if (runnableType != null) {
                            TriggerInterface_BoothApprovalDashboard.Runnable runnableClass;
                            try {
                                runnableClass = (TriggerInterface_BoothApprovalDashboard.Runnable) runnableType.newInstance();
                            } catch(Exception e) {
