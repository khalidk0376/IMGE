...
Description/Purpose: This triggers is for Account_Legal_Entity__c which should call from any class or trigger for any alteration of any Account_Legal_Entity__c's record.
It check for excluding profiles and record type for any trigger.
Test Class:  ManageUserPackage_Trig_Test
*/

trigger Trigger_Account_Legal_Entities on Account_Legal_Entity__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_Account_Legal_Entities.Trig trig = new TriggerInterface_Account_Legal_Entities.Trig();
    ...
    TriggerInterfaceHelper.initTriggerMapAndList('Account_Legal_Entity__c');
    ...
    for (Trigger__c t : listTriggers) {    
        ...
        if (t.Object__c == 'Account_Legal_Entity__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<Account_Legal_Entity__c>();
                trig.newList = new List<Account_Legal_Entity__c>();
                trig.oldMap = new Map<Id,Account_Legal_Entity__c>();
                trig.newMap = new Map<Id,Account_Legal_Entity__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_Account_Legal_Entities.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_Account_Legal_Entities.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_Account_Legal_Entities (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_Account_Legal_Entities (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)
