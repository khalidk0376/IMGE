...
Description/Purpose: This triggers is for Inserting and deleting the attachment on event Edition 
Test Class:  ManageAttachment_Trig_Test
*/


trigger Trigger_Attachment on Attachment(before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_Attachment.Trig trig = new TriggerInterface_Attachment.Trig();
    ...
    TriggerInterfaceHelper.initTriggerMapAndList('Attachment');
    ...
    for (Trigger__c t : listTriggers) {
           system.debug('testAttachment'+t);    
        Boolean runTrigger = false;
        if (t.Object__c == 'Attachment') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<Attachment>();
                trig.newList = new List<Attachment>();
                trig.oldMap = new Map<Id,Attachment>();
                trig.newMap = new Map<Id,Attachment>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_Attachment.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_Attachment.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
