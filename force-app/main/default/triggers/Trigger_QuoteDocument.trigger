...
trigger Trigger_QuoteDocument on SBQQ__QuoteDocument__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_QuoteDocument.Trig trig = new TriggerInterface_QuoteDocument.Trig();
    ...
    TriggerInterfaceHelper.initTriggerMapAndList('SBQQ__QuoteDocument__c');
    ...
    for (Trigger__c t : listTriggers) { 
        ...
        if (t.Object__c == 'SBQQ__QuoteDocument__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<SBQQ__QuoteDocument__c>();
                trig.newList = new List<SBQQ__QuoteDocument__c>();
                trig.oldMap = new Map<Id,SBQQ__QuoteDocument__c>();
                trig.newMap = new Map<Id,SBQQ__QuoteDocument__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_QuoteDocument.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_QuoteDocument.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_QuoteDocument (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_QuoteDocument (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)
