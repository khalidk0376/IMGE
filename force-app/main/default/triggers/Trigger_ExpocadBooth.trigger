...
Description/Purpose: This triggers is use for rent, hold, unhold , unrent and release.
*/


trigger Trigger_ExpocadBooth on ExpocadBooth__c(before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_ExpocadBooth.Trig trig = new TriggerInterface_ExpocadBooth.Trig();
    ...
    TriggerInterfaceHelper.initTriggerMapAndList('ExpocadBooth__c');
    ...
    if (TriggerInterfaceHelper.isRunning) { 
        for (Trigger__c t : listTriggers) {   
            Boolean runTrigger = false;
            if (t.Object__c == 'ExpocadBooth__c') {
                ...
                || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                    trig.oldList = new List<ExpocadBooth__c>();
                    trig.newList = new List<ExpocadBooth__c>();
                    trig.oldMap = new Map<Id,ExpocadBooth__c>();
                    trig.newMap = new Map<Id,ExpocadBooth__c>();
                    trig.triggerSize = 0;
                    ...
                    if (trig.triggerSize > 0) {
                        ...
                        if (runnableType != null) {
                            TriggerInterface_ExpocadBooth.Runnable runnableClass;
                            try {
                                runnableClass = (TriggerInterface_ExpocadBooth.Runnable) runnableType.newInstance();
                            } catch(Exception e) {
                            ...
                            if (runnableClass != null) {
                                ...
                                if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                    ...
                                    system.debug('********** Trigger_ExpocadBooths (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                    if (seconds > 10)
